...
| OQ-014 | Which **web stack** is chosen for the Portal Tracking Publik, given that Flutter is explicitly not mandatory there? | §5.4 permits a lighter stack if it performs materially better on low-end Android browsers. | Step 7 | The choice is recorded in a decision record in the Step that builds it; performance for this surface outranks stack uniformity. **STILL OPEN — AWAITING AN OWNER DECISION RECORD.** Step 7 implemented the zero-dependency option: the portal is server-rendered Blade at `/lacak/{token}`, adding NO new dependency, NO new toolchain, and NO third-party asset, with no script on the page at all. That is the most conservative answer available and it is reversible, but it is NOT a ratified product decision — an agent does not accept one on the owner's behalf (Rule 12, Rule 00 hard rule 6). The owner may ratify the Blade portal in a decision record, or direct a different stack, in which case only `backend/resources/views/tracking/` and one web route change. |
...
| OQ-018 | Is a **customer-initiated tracking OTP** an "urgent" message for quiet-hours purposes, or must it defer like every other message? | Master Source §14.1 rule 6 holds **non-urgent** messages until quiet hours end, and §14.2's notification catalogue contains **no OTP entry at all** — so the canonical text neither classifies an OTP nor grants it an exception. FR-091 requires OTP verification for the two sensitive portal actions, and a challenge lives five minutes, so a deferred OTP is in practice an OTP that is never sent. | Step 7 | **Step 7 implemented the CONSERVATIVE reading and defers, so no OTP is sent inside 20.00–08.00 outlet local time.** The consequence is stated rather than hidden: the FR-091 sensitive-action flow is unavailable during quiet hours, which is a real usability limitation. It is NOT resolved by invention — NOT-022 permits a quiet-hours exception only where the Master Source or an accepted decision record grants one, and neither does (Rule 00 hard rule 6). Any answer must keep opt-out, dedup, and the account-takeover rule (an OTP and a tracking link never in one message) intact; granting an exception would require a decision record naming exactly which message classes it covers, so that "urgent" does not become the route every future message takes. |
...
| Open questions resolved | **1** — OQ-017, by an owner decision recorded in [DEC-0036](../decisions/DEC-0036-oq-017-order-rounding-mode-halfup.md). OQ-001 … OQ-016 and OQ-018 await a decision. |
| Requirements depending on an unresolved open question | **1** — FR-091 (OTP-gated sensitive portal actions) currently defers during quiet hours under OQ-018's conservative reading, which makes the flow unavailable 20.00–08.00 outlet local time. The gate itself is implemented and tested; what awaits an owner decision is whether a customer-initiated OTP is "urgent". OQ-014 also awaits ratification of the portal's web stack, though Step 7 implemented the zero-dependency option and nothing depends on the answer being different. |
