...
| OQ-009 | Are **transactional messages subject to quiet hours without exception**, or is there a defined critical-message exception path? | §14.1 rule 6 defers non-urgent messages; §11.2 says reminders respect quiet hours; whether any message class is exempt is not stated. | Step 7, Step 9 | **STILL OPEN, and narrowed.** [DEC-0040](../decisions/DEC-0040-oq-018-user-initiated-security-transaction-quiet-hours-exemption.md) granted the first and so far only exception, for the single class `USER_INITIATED_SECURITY_TRANSACTION` — a customer-initiated FR-091 OTP. Whether any *other* message class is exempt is still undecided, and the conservative answer is unchanged: absent a further decision record, quiet hours apply to everything else, including every transactional order notification and the whole H+1/H+3/H+7/H+14 ladder. DEC-0040 is deliberately not a precedent for a general "urgent" category; it is gated on an observable explicit customer request, which no scheduled message has. |
...
| OQ-014 | Which **web stack** is chosen for the Portal Tracking Publik, given that Flutter is explicitly not mandatory there? | §5.4 permits a lighter stack if it performs materially better on low-end Android browsers. | Step 7 | **RESOLVED by [DEC-0041](../decisions/DEC-0041-oq-014-laravel-blade-as-the-public-tracking-portal-stack.md), 26 July 2026: the owner ratifies server-rendered Laravel Blade** at `/lacak/{token}`. This confirms the existing zero-dependency implementation — no new dependency, no new toolchain, no third-party asset, and no script on the page — and changes no behaviour. The record writes down the boundaries that keep it that way: Blade is for the public portal ONLY and never a parallel admin or operations application; token validation, tenant isolation, the customer-visible projection, masking, consent, and notification rules stay in canonical backend services and are never duplicated in a view; no persistent browser storage of the token and no public authentication session; and the transport controls (`no-store`, `noindex`, `Referrer-Policy`, CSP, anti-framing, escaping, rate limiting, one generic invalid-link response) remain mandatory. No Step 8 or Step 9 control may be added to the surface. Audited structurally by `scripts/validate-dec-0041-portal-stack.py`. |
...
| OQ-018 | Is a **customer-initiated tracking OTP** an "urgent" message for quiet-hours purposes, or must it defer like every other message? | Master Source §14.1 rule 6 holds **non-urgent** messages until quiet hours end, and §14.2's notification catalogue contains **no OTP entry at all** — so the canonical text neither classifies an OTP nor grants it an exception. FR-091 requires OTP verification for the two sensitive portal actions, and a challenge lives five minutes, so a deferred OTP is in practice an OTP that is never sent. | Step 7 | **RESOLVED by [DEC-0040](../decisions/DEC-0040-oq-018-user-initiated-security-transaction-quiet-hours-exemption.md), 26 July 2026: the owner classifies a customer-initiated OTP as a `USER_INITIATED_SECURITY_TRANSACTION`, exempt from quiet hours.** This is the exception `NOT-022` reserves to a decision record, and it is the first one granted — §14.1 rule 6 is unchanged for every non-urgent message. **It supersedes the conservative deferral Step 7 originally shipped**, which left FR-091 unavailable twelve hours a day. The exemption is gated on an explicit customer request and on nothing else: `OtpDispatchOrigin` is a required typed argument with no permissive default, an automated origin is REFUSED (`otp_not_customer_initiated`) rather than deferred, and the ordinary outbox refuses OTP-carrying templates outright. Marketing cannot acquire it — category comes from the template, never a caller. Rate limits, resend cooldown, expiry, attempt limit, single-use consumption, dedup, opt-out, and the account-takeover rule are all unchanged. Two database CHECK constraints make the boundary structural. |
...
| Open questions recorded | OQ-001 … OQ-018 |
| Open questions resolved | **3** — OQ-017 by [DEC-0036](../decisions/DEC-0036-oq-017-order-rounding-mode-halfup.md), OQ-018 by [DEC-0040](../decisions/DEC-0040-oq-018-user-initiated-security-transaction-quiet-hours-exemption.md), and OQ-014 by [DEC-0041](../decisions/DEC-0041-oq-014-laravel-blade-as-the-public-tracking-portal-stack.md). OQ-001 … OQ-013, OQ-015, and OQ-016 await a decision. |
| Requirements depending on an unresolved open question | **0** — FR-091 no longer depends on an open question: DEC-0040 classified the customer-initiated OTP and the FR-091 sensitive-action flow now works at every hour. OQ-009 (whether any *other* message class is exempt) remains open and its conservative answer is unchanged: absent a further decision record, quiet hours apply to everything else. |

No assumption in this document is a decision. No open question here has been closed by invention: OQ-017,
OQ-018, and OQ-014 were each resolved by the repository owner — in DEC-0036, DEC-0040, and DEC-0041
respectively — not by an agent, and the remaining fifteen still await an owner decision.
