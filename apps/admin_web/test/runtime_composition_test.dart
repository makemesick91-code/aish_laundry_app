...
void main() {
  group('production composition', () {
    ...
    test('uses session-cookie transport, and holds no token', () {
      final container = productionContainer();

      // Console Web must never hold a bearer token: script-readable storage on
      // a page is readable by any script that runs there, including an injected
      // one (Rule 38, hard rule 2). Its credential is an HttpOnly cookie this
      // code cannot read.
      expect(
